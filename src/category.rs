...
impl Category {
    ...
    pub async fn parent_categories(full: &str, _filter_hidden: bool) -> Vec<Category> {
        let params = &[
            ("action", "query"),
            ("prop", "categories"),
            ("clprop", "hidden"),
        ...
        let categories: Vec<Category> = res["query"]["pages"]
            ...
            .flat_map(|(_page_id, page)| {
                page["categories"]
                    ...
                    .filter_map(|c| Category::from_full(c["title"].as_str().unwrap()).ok())
            })
            .collect();
        categories
    }
    ...
    pub async fn subcategories(full: &str) -> Vec<Category> {
        ...
        let categories: Vec<Category> = res["query"]["categorymembers"]
            ...
            .filter_map(|v| Category::from_full(v).ok())
            .collect();
        categories
    }
        ...
        if let Some(r) = res["query"]["pages"].as_object() {
            r.iter()
                .filter_map(|(_, value)| {
                    ...
                    let r = Category::from_full(&title).ok()?;
                    Some(Category(pages_count, subcat_count, r.2))
                })
                .collect()
        } else {
            Vec::new()
    ...
    pub fn from_full(full: &str) -> Result<Self, ()> {
        ...
        let index = f.ok_or(())?;
        let title = full[index + 9..].to_owned();

        Ok(Category(0, 0, title))
    }
}
